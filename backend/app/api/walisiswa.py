...
@router.get("/{id_walisiswa}")
async def detail(id_walisiswa: int, db_session: AsyncSession = Depends(get_async_session)):
    response = {"status": "Success", "message_id": "00"}
    resp = await walisiswa_crud.get_detail_walisiswa(db_session=db_session, id_walisiswa=id_walisiswa)
...
@router.put("/edit")
async def edit(id_walisiswa: int, request: WaliSiswa, db_session: AsyncSession = Depends(get_async_session)) -> Dict[str, Any]:
    response = {"status": "Success", "message_id": "00"}
    resp = await walisiswa_crud.edit_walisiswa(id_walisiswa=id_walisiswa, request=request, db_session=db_session)
    response.update(resp)
    return response

@router.delete("/hapus/{id_walisiswa}")
async def hapus(id_walisiswa: int, db_session: AsyncSession = Depends(get_async_session)) -> Dict[str, Any]:
    response = {"status": "Success", "message_id": "00"}
    resp = await walisiswa_crud.delete_walisiswa(id_walisiswa=id_walisiswa, db_session=db_session)
    response.update(resp)
    return response
...
async def add_laporanpembayaran(request: JenisWali, db_session: AsyncSession = Depends(get_async_session)) -> Dict[str, Any]:
    response = {"status": "Success", "message_id": "00"}
...
async def edit_laporanpembayaran(id_laporanpembayaran: int, request: JenisWali, db_session: AsyncSession = Depends(get_async_session)) -> Dict[str, Any]:
    response = {"status": "Success", "message_id": "00"}

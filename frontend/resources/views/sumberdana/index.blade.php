<x-app-layout title="Data Sumber Dana">
    @if(session('alert'))
    ...
    <x-adminlte-card theme="lime" theme-mode="outline">
        @if (Auth::user()->role == '1')
            <x-submit-button method="POST" action="{{route('admin.sumber_dana.create')}}"
                             theme="success" label="Tambah Data" icon="fas fa-plus" type="submit"></x-submit-button>
        @elseif(Auth::user()->role == '2')
            <x-submit-button method="POST" action="{{route('bendahara.sumber_dana.create')}}"
                             theme="success" label="Tambah Data" icon="fas fa-plus" type="submit"></x-submit-button>
        @endif
